# -*- coding: utf-8 -*-
"""Resolve per-page feedback ``doc_id`` from ``.lbcf.yml``; otherwise defaults to esp-dev-kits's default doc IDs (5600 / 5601)."""
...
_DEFAULT_BY_LANG = {"en": "5600", "zh_CN": "5601"}

_feedback_maps_cache: Optional[Dict[str, Dict[str, str]]] = None


def _document_link_to_pagename(link: str) -> Optional[str]:
    """Strip IDF target prefix from published doc URL -> Sphinx pagename."""
    marker = "/latest/"
    i = link.find(marker)
    if i < 0:
        return None
    tail = link[i + len(marker) :]
