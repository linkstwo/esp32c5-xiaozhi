...
bool MqttProtocol::SendAudio(std::unique_ptr<AudioStreamPacket> packet) {
    ...
    *(uint32_t*)&nonce[12] = htonl(++local_sequence_);
    ...
    if ((local_sequence_ % 20) == 0) {
        ESP_LOGI(TAG, "UDP audio sent: seq=%lu opus=%u encrypted=%u sent=%d",
                 (unsigned long)local_sequence_, packet->payload.size(), encrypted.size(), sent);
    }
    return sent > 0;
}
...
bool MqttProtocol::OpenAudioChannel() {
    ...
    if (!SendText(message)) {
        ESP_LOGW(TAG, "Hello publish failed, restarting MQTT client and retrying once");
        if (!StartMqttClient(true)) {
            return false;
        }
        if (!SendText(message)) {
    ...
    udp_->OnMessage([this](const std::string& data) {
        ...
        if (on_incoming_audio_ != nullptr) {
            if ((sequence % 20) == 0) {
                ESP_LOGI(TAG, "UDP audio received: seq=%lu opus=%u sample_rate=%d duration=%d",
                         (unsigned long)sequence, packet->payload.size(), server_sample_rate_, server_frame_duration_);
            }
