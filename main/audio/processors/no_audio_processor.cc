...
void NoAudioProcessor::Initialize(AudioCodec* codec, int frame_duration_ms, srmodel_list_t* models_list) {
    ...
    ESP_LOGI(TAG, "Raw mic passthrough initialized: frame_samples=%d", frame_samples_);
}
...
void NoAudioProcessor::Start() {
    is_running_ = true;
    ESP_LOGI(TAG, "Raw mic passthrough started");
}

void NoAudioProcessor::Stop() {
    is_running_ = false;
    output_buffer_.clear();
    ESP_LOGI(TAG, "Raw mic passthrough stopped");
}
